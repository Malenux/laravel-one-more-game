...
<section class="admin-table">
    ...
    <div class="table__header">
        <div class="table__header__box">
            <button class="filter-button" data-endpoint="{{ route('customers') }}">
        ...
        @if($records->isEmpty())
            <div class="table-row">
                <div class="table-row__content">
                    <label>No hay registros en esta tabla</label>
                </div>
            </div>
        @else
            @foreach($records as $record)
                <div class="table-row">
                    <div class="element-box__upper-row">
                        <button class="edit-button" data-endpoint="{{ route('customers_edit', $record->id ?? $record->_id) }}">
                        ...
                        <button class="delete-button" data-endpoint="{{ route('customers_destroy', $record->id ?? $record->_id) }}">
    ...
    <div class="table__footer">
        <div class="table__footer-box">
            <div class="table-page-info">
                {{ $records->count() > 0 ? $records->total() : '0' }} registros en total,
                mostrando {{ $records->perPage() > 0 ? $records->perPage() : '0' }} por página
            </div>
        </div>
        <div class="table-page-controls">

            <button class="table-pagination-page first-page {{ $records->onFirstPage() ? 'inactive' : '' }}" data-pagination="{{ $records->url(1) }}">
            ...
            <button class="table-pagination-page prev-page {{ $records->onFirstPage() ? 'inactive' : '' }}" data-pagination="{{ $records->previousPageUrl() }}">
            ...
            <button class="table-pagination-page next-page {{ $records->hasMorePages() ? '' : 'inactive' }}" data-pagination="{{ $records->nextPageUrl() }}">
            ...
            <button class="table-pagination-page last-page {{ $records->hasMorePages() ? '' : 'inactive' }}" data-pagination="{{ $records->url($records->lastPage()) }}">
